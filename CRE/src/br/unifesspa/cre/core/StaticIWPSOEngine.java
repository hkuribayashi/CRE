...
public class StaticIWPSOEngine extends PSOEngine {

	public StaticIWPSOEngine(double alpha, double beta, CREEnv env, Integer swarmSize) {
		super(alpha, beta, env, swarmSize);
	}

	public StaticIWPSOEngine(double alpha, double beta, Scenario scenario, Integer swarmSize) {
	...
	public void run() {
		...
		StaticIWPSO[] pso = new StaticIWPSO[this.env.getSimulations()];
		for (int i=0; i<pso.length; i++) {
			pso[i] = new StaticIWPSO(this.alpha, this.beta, this.scenario.clone(), this.env.getPsoSteps(), this.swarmSize, targetSolution);
		}
